...
// Clerk-gated (dealer) export of the room PDF leave-behind. Note the middleware
// anchor: "/api/room/(.*)" does NOT match "/api/rooms/..." — this stays protected.
export async function POST(
  ...
  if (!env.VIEWING_ROOM_RENDER_SECRET) {
    return NextResponse.json(
      { error: "VIEWING_ROOM_RENDER_SECRET is not configured" },
      { status: 500 },
    );
  }
