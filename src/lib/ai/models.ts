...
// The models the Settings UI offers per provider — the selectable catalog. A
// model must be listed here (under an implemented provider) to be chosen in the
// UI or saved to the DB. Grow this as new models ship; the code default may
// point at any entry here. `hint` powers the dropdown's secondary line.
...
export const MODEL_CATALOG: Record<AiProvider, CatalogModel[]> = {
  anthropic: [
    { id: "claude-opus-4-8", label: "Claude Opus 4.8", hint: "Most capable — best for hard extraction and judgment" },
    { id: "claude-sonnet-5", label: "Claude Sonnet 5", hint: "Balanced speed and quality" },
...
// A flat list of every selectable model under an IMPLEMENTED provider — what the
// UI renders and what save-time validation checks against. `model` is the id to
// persist; `label`/`hint` are display only.
...
export function selectableModels(): SelectableModel[] {
  ...
  for (const provider of AI_PROVIDERS) {
    ...
    for (const m of MODEL_CATALOG[provider]) {
      out.push({ provider, model: m.id, label: m.label, hint: m.hint });
    }
  }
  return out;
}

// Look up catalog metadata for a resolved model, if it's in the catalog. A model
// set via env to something off-catalog resolves fine but returns null here.
export function catalogEntry(sel: FeatureModel): CatalogModel | null {
  return MODEL_CATALOG[sel.provider]?.find((m) => m.id === sel.model) ?? null;
}

// Save-time validation: the selection must name an implemented provider AND a
// model in that provider's catalog. Returns the normalized selection or throws
// AiConfigError — used by the Settings save action so a bad write can't persist.
export function assertSelectableModel(provider: string, model: string): FeatureModel {
  if (!AI_PROVIDERS.includes(provider as AiProvider)) {
    throw new AiConfigError(`Unknown AI provider "${provider}".`);
  }
  const p = provider as AiProvider;
  if (!IMPLEMENTED_PROVIDERS.has(p)) {
    throw new AiConfigError(
      `Provider "${provider}" has no implemented client adapter yet. Implemented: ${[...IMPLEMENTED_PROVIDERS].join(", ")}.`,
    );
  }
  if (!MODEL_CATALOG[p].some((m) => m.id === model)) {
    throw new AiConfigError(`Model "${model}" is not in the ${provider} catalog.`);
  }
...
// Pure precedence merge: given a feature, an optional stored (DB) selection, and
// the env source, return the effective model. Precedence is
//   DB selection  >  env override  >  code default
// The stored selection wins because it's what the dealer actively chose in the
// UI; env/code are the seed defaults for any feature never touched there. The
// implemented-provider guard applies whichever layer won. Pure + synchronous so
// the precedence rule is unit-testable; the DB read lives in ai/settings.ts.
export function pickEffectiveModel(
  feature: AiFeature,
  stored: FeatureModel | null | undefined,
  options: ResolveOptions = {},
): FeatureModel {
  if (stored) {
    if (!IMPLEMENTED_PROVIDERS.has(stored.provider)) {
      throw new AiConfigError(
        `AI feature "${feature}" has a stored provider "${stored.provider}" with no implemented client adapter yet. ` +
          `Implemented providers: ${[...IMPLEMENTED_PROVIDERS].join(", ")}.`,
      );
    }
    return stored;
  }
  return resolveFeatureModel(feature, options);
}
